...
object ShortestPath {
    ...
    // Modfied by  : Sharmodeep Sarkar  
	// Original Author : Nat Tuck
        @tailrec
    def iterate(nodes : RDD[Node], sc : SparkContext) : RDD[Node] = {
        val links = nodes.flatMap(nextLinks)
        val neighbours = nodes.keyBy(_._1).cogroup(links).map(nextNode)
        val chkDone = sc.accumulator(0,"chkDone?")
        neighbours.foreach(xx => {
        							val(name,_,_,dist) = xx
        							if(name==dst && dist < Double.PositiveInfinity){
												chkDone+=dist.toInt
											}
        							})
        if(chkDone.value > 0){
