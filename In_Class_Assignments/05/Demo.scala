...
object Demo {
    def main(args: Array[String]) {
        ...
        // this is modified from 2 reducers to 1 reducer
...
// this is changed to handle Batting.csv
...
class NamesMapper extends Mapper[Object, Text, Text, Text] {
    ...
    override def map(_k: Object, vv: Text, ctx: Context) {
        ...
        val year = cols(1)
        val homeRuns = cols(11)

        if (playerID != null && playerID != "" && year != null && year != "") {
            ctx.write(new Text(playerID+"\t"+year), new Text("homeRuns=" + homeRuns))
        }
    }
}

// this is changed to handle Salaries.csv
