 ...
 */
public class Cluster_Analysis_Average {
    ...
    private boolean sanityTest(String[] input) {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			Double sumArr[] = new Double[12];
			Integer countArr[] = new Integer[12];
			Double avgArr[] = new Double[12];
                        Arrays.fill(sumArr,0.0);
                        Arrays.fill(countArr,0);
                        Arrays.fill(avgArr,0.0);
			Boolean isActive = false;
			for (Text val : values) {
				String valStr = val.toString();
				String splitArr[] = valStr.split("-");
				int month = Integer.parseInt(splitArr[1]);
				String year = splitArr[0];
				Double averagePrice = Double.parseDouble(splitArr[2]);
                                if (year.equals("2015"))
					isActive = true;
				sumArr[month - 1] += averagePrice;
				countArr[month - 1] += 1;				
			}
			...
			for (int i = 0; i < 12; i++) {
                            totalFlights += countArr[i];
                            if (sumArr[i] != 0 || countArr[i] != 0) {
				avgArr[i] = (sumArr[i] / countArr[i]);
                            } 
                        }
                        

			Text reducerrKey = new Text();
			if (isActive) {
                            for (int j = 0; j < 12; j++) {
                                String keyString = (j + 1) + "\t" + avgArr[j] + "\t" + totalFlights + "";
                                reducerrKey.set(keyString);
                                context.write(key, reducerrKey);
                            }
			}
		}
	}
